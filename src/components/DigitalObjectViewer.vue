<template>
  <div v-if="digitalObject" class="space-y-4 w-full">
    <div class="flex w-full">
      <a class="w-full" :href="digitalObject.url" rel="noopener noreferrer" title="View Digital Object">
        <img 
          :src="digitalObject.reference_url || digitalObject.url" :alt="title || digitalObject.filename" 
    ...
    <div>
      ...
      <p v-if="referenceCode" class="text-sm text-gray-500">{{ referenceCode }}</p>
      <div class="flex items-center text-sm text-gray-600 space-x-1">
        <span v-html="levelIcon" class="w-4 h-4"></span>
        <span>{{ levelOfDescription }}</span>
        <span>
          &middot; {{ publicationStatus === 'Published' ? 'Publicado' : 'Não publicado' }}
          &middot; em <a href="#" class="text-blue-600 hover:underline" @click.prevent="viewParent">{{ parentTitle }}</a>
        </span>
      </div>
      <p class="text-xs text-gray-500">
        {{ digitalObject.media_type }} ({{ digitalObject.mime_type }}) &middot; {{ Number(digitalObject.byte_size / (1024 * 1024)).toFixed(2) }} MB
      </p>
      <p class="mt-2 text-xs text-gray-500">
        Enviada em {{ new Date(digitalObject.uploaded_at).toLocaleDateString() }}
      </p>
...
export default {
  ...
  methods: {
    describeObject() {
      this.$router.push({
        name: 'Describe',
        params: { pathMatch: [this.slug] }
      });
    },
    viewParent() {
      this.$router.push({
        name: 'Browse',
        params: { pathMatch: this.parent }
      });
    }
  }
};
